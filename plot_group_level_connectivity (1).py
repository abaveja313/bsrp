...
precision_measure = ConnectivityMeasure(kind='precision')
###############################################################################
# and repeat the previous operation.
partial_correlation_matrices = partial_correlation_measure.fit_transform(
    adhd_subjects)
covariance_matrices = covariance_measure.fit_transform(adhd_subjects)
precision_matrices = precision_measure.fit_transform(adhd_subjects)
###############################################################################
...
for kind in kinds:
    ...
    cv_scores = cross_val_predict(svc, connectivity_biomarkers[kind],
                                y=adhd_labels, cv=cv)
    print '-' * 30
    print "Kind:", kind
    print '\n',  confusion_matrix(adhd_labels, cv_scores), '\n'
    print "score:", accuracy_score(adhd_labels, cv_scores), '\n'
    print "Classification Report:", classification_report(adhd_labels, cv_scores, target_names=['no adhd', 'adhd'])
    print '\n'
    print list(cv_scores)
    print list(adhd_labels)
